...
test('wraps task and properly formats and throws error', async () => {
  ...
  await wrappedTask(ctx).catch((error) => {
    err = error;
  });
  ...
  expect(formatLogMessageOneLine.mock.calls[0][0].error.message).toBe(
    'Task failed',
    'log message error has original error message',
  );
});
